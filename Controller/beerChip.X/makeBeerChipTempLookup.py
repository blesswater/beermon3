...
if __name__ == "__main__":
    ...
    for sampleIndex in range(0, 1024):
        ...
        if( (sampleIndex % 256) == 0 ):
            print( 'uint8_t tempLookupMsb%d( uint8_t index )\n{' % (batch256) )
            print( '\n' )
            print( '    asm( "MOVF tempLookupMsb%d@index, W" );' % (batch256) )
            print( '    asm( "BRW");' )
            batch256 += 1

        cmtStr = "Sample = %d(0x%04X), Vrt = %f, Rt = %f, F = %f, 8.8 = 0x%04X" % (sampleIndex, sampleIndex, Vrt, Rt, F, EDotE)
        print( '    /* %s */' % (cmtStr) )
        print( '    asm( "RETLW 0x%02X" );' % ((EDotE & 0x0000FF00) >> 8) )
    ...
    for sampleIndex in range(0, 1024):
        ...
        if( (sampleIndex % 512) == 0 ):
            print( 'uint8_t tempLookupLsb%d( uint8_t index )\n{' % (batch256) )
            print( '\n' )
            print( '    asm( "MOVF tempLookupLsb%d@index, W" );' % (batch256) )
            # print( '    asm( "RRF tempLookupLsb%d@index, W" );' % (batch256) )
            print( '    asm( "BRW");' )
            batch256 += 1
        ...
        if( sampleIndex & 0x0001 ):
            ...
            print( '    /* Index %d(0x%04X) */' % (thisIndex, thisIndex) )
            print( '    asm( "RETLW 0x%02X" );' % (thisWord) )
            thisWord = 0x00
        else:
